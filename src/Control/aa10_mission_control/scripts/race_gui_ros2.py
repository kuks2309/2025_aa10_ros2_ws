...
# Find UI file - try multiple locations
def find_ui_file():
    # Try 1: Same directory as script (for development)
    script_dir = os.path.dirname(os.path.abspath(__file__))
    ui_file = os.path.join(script_dir, 'race_gui.ui')
    if os.path.exists(ui_file):
        return ui_file

    # Try 2: Installed location (for ros2 run)
    try:
        from ament_index_python.packages import get_package_share_directory
        package_share_dir = get_package_share_directory('aa10_mission_control')
        ui_file = os.path.join(package_share_dir, 'scripts', 'race_gui.ui')
        if os.path.exists(ui_file):
            return ui_file
    except:
        pass
    ...
    return os.path.join(script_dir, 'race_gui.ui')
...
class ROS2RaceGUINode(Node):
    def __init__(self):
        ...
        self.yaw_control_mode_pub = self.create_publisher(Int8, "/car_control/steering_control_mode", qos_profile)
        self.lane_control_flag_pub = self.create_publisher(Bool, "/flag/lane_control_set", qos_profile)
        self.car_odom_reset_pub = self.create_publisher(Bool, "/car_control/reset_odom", qos_profile)
        self.pub_traffic_sign = self.create_publisher(String, '/traffic_light', qos_profile)
        self.target_angle_pub = self.create_publisher(Float32, "/car_control/target_angle", qos_profile)
