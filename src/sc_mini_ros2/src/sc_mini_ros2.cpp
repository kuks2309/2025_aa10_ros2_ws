...
int main(int argc, char **argv)
{
		...
		auto node = rclcpp::Node::make_shared("sc_mini_ros2");
		...
		auto scan_publish = std::make_shared<sensor_msgs::msg::LaserScan>();

		const char *port_t = port.data();
		fd = open_port(port_t, baud_rate);
		
		if(fd < 0) {
			RCLCPP_ERROR(node->get_logger(), "Failed to open port %s", port.c_str());
			return -1;
		}
		
		RCLCPP_INFO(node->get_logger(), "Port: %s, Baud rate: %d", port.c_str(), baud_rate);
		
		sc_m_c::SCLaser laser;
		rclcpp::Rate loop_rate(40);  // 40Hz
		
		while (rclcpp::ok())
		{   
			if(laser.poll(scan,fd) == 0)
			{
				laser.PointCloudFilter(scan);
				laser.angle_insert(scan,scan_publish);
				scan_publish->header.frame_id = frame_id;
				scan_publish->header.stamp = node->now();
